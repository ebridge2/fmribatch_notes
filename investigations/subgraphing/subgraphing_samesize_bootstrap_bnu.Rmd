---
title: "Signal Subgraphing for Batch Effect Detection with Parametric Bootstrap Null"
...
#model.null.bootstrap <- function(samp, Y, )
  ...
  for (i in 1:nrep) {
    ...
    for (j in 1:length(zset)) {
      ...
      for (k in 1:length(global.est$classes)) {
        ...
        dats[[k]] <- sg.bern.sample_graph(Pclass[,,k], s=ns[k])
        labs[[k]] <- array(global.est$classes[k], dim=c(ns[k]))
      }
...
dwi.result <- parametric_bootstrap(dwi.bin_graphs, dwi.sexs, dwi.datasets, verb=FALSE, nrep=nrep, tstat=tstat.jaccard)
```

we first visualize our set of subgraphs computed from each study using the sex class label and study as the batch label (the alternative model):
...
ggplot(data=dat, aes(t, group=sample, color=sample)) +
  ...
  ggtitle(TeX(sprintf("dMRI Bootstrap Test Statistics, $p$ = %.3f", dwi.result$p)))
```
...
ggplot(data=countdat, aes(x=nds, y=ncount, fill=hypothesis)) +
  ...
  ggtitle("dMRI Edge Count Bootstrap Null")
```
...
fmri.result <- parametric_bootstrap(fmri.bin_graphs, fmri.sexs, fmri.datasets, verb=FALSE, nrep=nrep, tstat=tstat.jaccard)
```
...
ggplot(data=dat, aes(t, group=sample, color=sample)) +
  ...
  scale_x_continuous(limits=c(0, 1)) +
  theme(panel.background=element_rect(fill="#ffffff")) +
  ggtitle(TeX(sprintf("fMRI Bootstrap Test Statistics, $p$ = %.3f", fmri.result$p)))
```
...
ggplot(data=countdat, aes(x=nds, y=ncount, fill=hypothesis)) +
  ...
  ylab("Normalized  Count") +
  xlab("Number of Datasets Edge Appears in Subgraph For") +
  ggtitle("fMRI Edge Count Bootstrap Null")
```
...
ggplot(across.delta, aes(delta, group=modality, color=modality, fill=modality)) +
  ...
  xlab(TeX("$\\delta$")) +
  ylab(TeX("Density")) +
  ggtitle("Bootstrap Across Modality Comparison") +
