...
result <- parametric_bootstrap(dwi.bin_graphs, dwi.sexs, dwi.datasets, verb=FALSE, nrep=100, tstat=tstat.jaccard)
```
...
plot.zset(result$zset, result$alt.ssgs)
```

and we visualize the synthetically generated sub-graphs for each dataset from our synthetic bootstrap:

```{r, fig.width=8, fig.height=7}
plot.zset(result$zset, result$null.ssgs)
```

Along with the globally estimated sub-graph:

```{r, fig.height=4, fig.width=5}
fmriu.plot.plot_graph(result$global.ssgs,  legend.name=TeX("$I_{uv}$"),
                        title="Global SSG",
                        limits=c(min(result$global.ssgs), max(result$global.ssgs)), xlabel = "ROI",
                        ylabel = "ROI") + theme(legend.position=NaN)
```
...
vline = data.frame(x=result$tstat.alt, type="t")

dat <- data.frame(sample="null", t=result$tstat.null)
...
ggplot(data=dat, aes(t, group=sample, color=sample)) +
  ...
  scale_linetype_manual(values=c("dashed"), name="Cutoff", breaks=c("t"),
                        labels=lapply(sprintf("$\\tau_{observed} = %.3f", result$tstat.alt), TeX)) +
  xlab(TeX("$\\tau$")) +
  ylab("Density") +
  scale_x_continuous(limits=c(0, 1)) +
  theme(panel.background=element_rect(fill="#ffffff")) +
  ggtitle(TeX(sprintf("dMRI Bootstrap Test Statistics, $p$ = %.3f", result$p)))
```
...
result <- parametric_bootstrap(fmri.bin_graphs, fmri.sexs, fmri.datasets, verb=FALSE, nrep=100, tstat=tstat.jaccard)
```
