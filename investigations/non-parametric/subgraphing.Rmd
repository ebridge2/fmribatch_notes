...
and also, the globally estimated sub-graph with edges $\mathbb{I}\{(u, v) \in \mathcal{S}\}$. Assuming the batch-effect is small, we intuitively expect that the two should look relatively similar:
...
plots[[1]] <- fmriu.plot.plot_graph(local_ssg,  legend.name=TeX("$M_{uv}"),
                      ...
                      ylabel = "ROI")
plots[[2]] <- fmriu.plot.plot_graph(dwi.global.ssg,  legend.name=TeX("$I\\{(u, v) \\in S\\}$"),
...
plots[[1]] <- fmriu.plot.plot_graph(local_ssg,  legend.name=TeX("$M_{uv}"),
                      ...
                      limits=c(min(local_ssg), max(local_ssg)), xlabel = "ROI",
                      ylabel = "ROI")
plots[[2]] <- fmriu.plot.plot_graph(fmri.global.ssg,  legend.name=TeX("$I\\{(u, v) \\in S\\}$"),
