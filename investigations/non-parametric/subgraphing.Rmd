...
counter <- 1
for (i in 1:length(dwi.dsets)) {
  dset_plots[[counter]] <- fmriu.plot.plot_graph(dwi.dset.ssgs[,,i],  legend.name=TeX("$I_{uv}$"),
                      title=dwi.dsets[i],
                      limits=c(min(dwi.dset.ssgs), max(dwi.dset.ssgs)), xlabel = "ROI",
                      ylabel = "ROI") + theme(legend.position=NaN)
  counter <- counter + 1
}

multiplot(plotlist = dset_plots, cols = ceiling(sqrt(length(dwi.dsets))))
```
...
and also, the local and globally estimated sub-graph with edges $I_{uv} = \mathbb{I}\{(u, v) \in \mathcal{S}\}$. Assuming the batch-effect is small, we intuitively expect that the two should look relatively similar:

```{r, fig.width=12, fig.height=12}
dset_plots <- list()

counter <- 1
for (i in 1:length(fmri.dsets)) {
  dset_plots[[counter]] <- fmriu.plot.plot_graph(fmri.dset.ssgs[,,i],  legend.name=TeX("$I_{uv}$"),
                      title=fmri.dsets[i],
                      limits=c(min(fmri.global.ssg), max(fmri.global.ssg)), xlabel = "ROI",
                      ylabel = "ROI") + theme(legend.position=NaN)
  counter <- counter + 1
}

multiplot(plotlist = dset_plots, cols = round(sqrt(length(fmri.dsets))))
```
