...
graphobj <- fmriu.io.collection.open_graphs(basepath='/data/raw_fmri_graphs/', datasets = dsets, atlases = c('desikan-2mm'), fmt='edgelist', rtype='array')
...
sexs = sexs[!is.nan(ages) & !is.nan(sexs)]
datasets = datasets[!is.nan(ages) & !is.nan(sexs)]
subjects = subjects[!is.nan(ages) & !is.nan(sexs)]
ages = ages[!is.nan(ages) & !is.nan(sexs)]
```

```{r}
VC = array(NaN, dim=c(70,70, 6))
pval = array(NaN, dim=c(70, 70))
test_dat = data.frame(sex=sexs, dataset=datasets, subject=subjects, age=ages, dat=array(NaN, dim=c(length(subjects))))
for (i in 1:70) {
  for (j in 1:70) {
    test_dat$dat <- graphs[,i,j]
    result <- anovaMM(dat~(subject)*(sex)*(dataset), test_dat)
  }
}
```